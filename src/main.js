...
import { 
    ...
    SATELLITE_CONFIGS, ENEMY_SPRITE_FILES
} from './config.js';
        ...
        function isBossWave(wave) {
            return wave >= config.BOSS_FIRST_WAVE && (wave - config.BOSS_FIRST_WAVE) % config.BOSS_ROUND_INTERVAL === 0;
        }
        ...
        function populateSpaceEncyclopedia() {
            ...
            satelliteContainer.innerHTML = Object.entries(config.SATELLITE_CONFIGS).map(([type, cfg]) => {
            ...
            const enemyCards = config.ENEMY_PROFILES.map(profile => {
            ...
            const bossCards = config.BOSS_SHIPS.map((boss, index) => renderEncyclopediaCard({
                title: boss.name,
                ...
                stats: [
                    ...
                    ['Appears', `Wave ${config.BOSS_FIRST_WAVE + index * config.BOSS_ROUND_INTERVAL}`]
                ]
            }));
            ...
            directivesContainer.innerHTML = config.COMMAND_DIRECTIVES
                .slice()
        ...
        function getDirectiveDefinition(id) {
            return config.COMMAND_DIRECTIVES.find(directive => directive.id === id);
        }
        ...
        function getDirectiveEffectValue(effect, fallback = null) {
            const directive = config.COMMAND_DIRECTIVES.find(item => item.effect === effect);
        ...
        function spawnBossMinion() {
            ...
            function spawnSingle(x, y) {
                const profiles = [config.ENEMY_PROFILES[4], config.ENEMY_PROFILES[5], config.ENEMY_PROFILES[0], config.ENEMY_PROFILES[1]];
        ...
        function spawnGatekeeper() {
            const playTop = getBossPlayTop();
            const profile = config.ENEMY_PROFILES[6];
            const spriteScale = ENEMY_SPRITE_SCALE[profile.type] || 3.2;
            ...
            const escProfile = config.ENEMY_PROFILES[4];
        ...
        function killBossModeEnemy(eIndex) {
            ...
            if ((enemy.category === 'ship' || enemy.category === 'miniboss' || enemy.category === 'gatekeeper') && Math.random() < (enemy.category === 'gatekeeper' ? 1.0 : 0.25)) {
                state.bossMode.drops.push({ x: enemy.x, y: enemy.y, type: 'weapon', enhancement: config.BOSS_WEAPON_ENHANCEMENTS[Math.floor(Math.random() * config.BOSS_WEAPON_ENHANCEMENTS.length)] });
        ...
        function openBossRewardCards() {
            ...
            const eligible = config.COMMAND_DIRECTIVES.filter(directive => getDirectiveLevel(directive.id) < 5);
            const shuffled = [];
            const pool = [...eligible];
            while (shuffled.length < 3 && pool.length > 0) {
                const totalWeight = pool.reduce((sum, directive) => sum + (config.DIRECTIVE_RARITY_WEIGHTS[directive.rarity] || 1), 0);
                let roll = Math.random() * totalWeight;
                let selectedIndex = 0;
                for (let i = 0; i < pool.length; i++) {
                    roll -= config.DIRECTIVE_RARITY_WEIGHTS[pool[i].rarity] || 1;
        ...
        function handleEnemyCollisions(enemy, i) {
            ...
            if (enemy.hp <= 0) {
                ...
                if (enemy.type === 'Rogue Comet') {
                    ...
                    state.cosmicData += 5;
                    const pool = config.COMMAND_DIRECTIVES.filter(d => getDirectiveLevel(d.id) < 5);
                    if (pool.length > 0) {
        ...
        function startNextWave() {
            ...
            while (baseBudget > 0) {
                let valid = pool.filter(i => config.ENEMY_PROFILES[i].threatCost <= baseBudget);
                if (valid.length === 0) valid = [pool[0]];
                let picked = valid[Math.floor(Math.random() * valid.length)];
                let cost = config.ENEMY_PROFILES[picked].threatCost || 1;
        ...
        function populateBossAttackPanel() {
            ...
            if (!container) return;
            container.innerHTML = config.BOSS_SHIPS.map((boss, index) => `
            ...
            config.BOSS_SHIPS.forEach((boss, index) => {
                bindButton(`boss-attack-btn-${index}`, () => {
                    ...
                    state.game.wave = config.BOSS_FIRST_WAVE + index * config.BOSS_ROUND_INTERVAL;
                    startBossMode();
                });
            });
        }
